...
export function BarjungApp({ repository }: { repository?: BarjungRepository } = {}) {
  ...
  const updateTargets = useCallback((targets: DistributionTarget[]) => {
    if (!distributionId || !snapshot) return;
    const current = snapshot.properties.find((property) => property.id === distributionId);
    if (current) actions.replaceProperty({ ...current, targets });
  }, [distributionId, snapshot, actions]);
  ...
  const distribution = distributionId ? properties.find((property) => property.id === distributionId) ?? null : null;
  ...
  return (
    <div className="app-shell">
      ...
      {selected && <PropertyDetail property={selected} office={office} mode={mode} onClose={() => setSelectedId(null)} onPublish={() => { setDistributionId(selected.id); setSelectedId(null); }} onEdit={() => { setFormError(""); setEditingPropertyId(selected.id); setSelectedId(null); }} />}
      {wizard && <PropertyWizard mode={mode} employees={employees} onClose={() => setWizard(false)} onFinish={async (input) => { const created = await actions.createProperty(input); setWizard(false); setSelectedId(created.id); showToast("새 매물을 등록했습니다."); }} />}
      {distribution && <DistributionModal property={distribution} mode={mode} agent={agent} onClose={() => setDistributionId(null)} onUpdate={updateTargets} requestDistribution={actions.requestDistribution} getProperty={actions.getProperty} />}
      {form && (
