...
describe("DistributionModal 전체 발행", () => {
  it("다섯 플랫폼 종료 후 완료 알림과 플랫폼별 완료 상태를 표시한다", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "선택 플랫폼 발행 시작 (5)" }));
    ...
    expect(screen.getAllByText("발행 완료")).toHaveLength(5);
  });
  ...
  it("실패 플랫폼 사유를 남기고 그 플랫폼만 재발행한다", async () => {
    ...
    expect(screen.getAllByText("이번 발행에서 선택하지 않음")).toHaveLength(2);
  });
  ...
  it("기본 전체 선택에서 원하는 플랫폼만 골라 명시적으로 발행을 시작한다", async () => {
    ...
    for (const name of ["인스타", "당근", "틱톡"]) fireEvent.click(screen.getByRole("checkbox", { name: `${name} 발행 선택` }));
    expect(requestDistribution).not.toHaveBeenCalled();
    fireEvent.click(screen.getByRole("button", { name: "선택 플랫폼 발행 시작 (2)" }));
    await waitFor(() => expect(requestDistribution).toHaveBeenCalledWith(properties[0].id, ["naver", "youtube"]));
  });
});
