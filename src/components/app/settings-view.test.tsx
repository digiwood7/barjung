...
describe("SettingsView Naver login", () => {
  it("플랫폼별 로그인 상태를 다섯 개의 카드로 표시한다", async () => {
    vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL) => new Response(JSON.stringify(
      String(input).includes("instagram") ? { status: "action_required", message: "로그인 필요" } : { status: "connected", message: "정상 연결" },
    ), { status: 200 })));
    render(<SettingsView {...props} connections={[
      ...
      { platform: "tiktok", status: "not_configured", lastCheckedAt: null },
      { platform: "youtube", status: "not_configured", lastCheckedAt: null },
    ]} />);

    const section = await screen.findByRole("region", { name: "플랫폼 로그인 상태" });
    expect(within(section).getAllByRole("article")).toHaveLength(5);
    expect(within(section).getAllByRole("button", { name: /상태 확인/ })).toHaveLength(5);
  });
  ...
  it("로컬 확인 필요 상태에서도 로그인 버튼으로 갱신을 요청한다", async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => new Response(JSON.stringify(
      String(input).includes("instagram")
        ? { status: "action_required", message: "인스타 로그인 필요" }
        : init?.method === "POST"
  ...
  it("로그인 연결 상태에서는 완료 버튼을 비활성화한다", async () => {
    ...
    const naverCard = (await screen.findByLabelText("네이버")).closest("article");
    expect(naverCard).not.toBeNull();
    expect(within(naverCard as HTMLElement).getByRole("button", { name: "로그인 완료" })).toBeDisabled();
  });

  it("인스타 로그인 요청을 Windows 실행기로 보낸다", async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => new Response(JSON.stringify(
      String(input).includes("instagram") && init?.method === "POST"
        ? { status: "action_required", message: "Windows 실행기에 인스타 로그인 요청을 보냈습니다." }
        : { status: "action_required", message: "로그인 필요" },
    ), { status: 200 }));
    vi.stubGlobal("fetch", fetchMock);
    render(<SettingsView {...props} />);

    fireEvent.click(await screen.findByRole("button", { name: "인스타 로그인" }));

    expect(await screen.findByRole("button", { name: "로그인 진행 중" })).toBeDisabled();
    expect(fetchMock).toHaveBeenCalledWith("/api/instagram/session", { method: "POST" });
  });
});
