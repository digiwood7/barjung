...
describe("workspace 데이터 서비스 (가짜 Supabase)", () => {
  ...
  it("배포 요청은 4개 플랫폼 target 을 queued 로 만들고 1분 안 중복을 거부한다", async () => {
    ...
    // 실행기가 처리한 결과가 화면 모델에 반영된다
    const target = db.rows("distribution_targets").find((t) => t.platform === "naver")!;
    target.status = "not_configured"; target.error_summary = "naver Playwright 어댑터를 고객 PC에서 연결해야 합니다.";
    const after = await getProperty(ctx, created.id);
    expect(after?.targets[0]).toMatchObject({ platform: "naver", status: "not_configured", progress: 100 });
    // 개별 플랫폼 재시도: 새 작업은 naver 만 queued, 나머지 플랫폼 상태는 이전 작업 값을 유지한다
    const retried = await requestDistribution(ctx, created.id, ["naver"]);
    expect(db.rows("distribution_jobs")).toHaveLength(2);
    expect(retried.targets.map((t) => t.status)).toEqual(["queued", "queued", "queued", "queued"]);
    expect(db.rows("distribution_targets").filter((t) => t.distribution_job_id === db.rows("distribution_jobs")[1].id)).toHaveLength(1);
  });
